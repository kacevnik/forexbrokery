...
$table .= '<div class="fbp_clear"></div>
	...
	$dg[3] .= '<div class="fbp_rat_result" title="Рейтинг форекс брокера '.$fb->fname.'">'. $fb->frating .'</div></div></div>';
		...
		if($distable['enable']['fdc4']==1){
	$dg[4] = '<div class="fbp_btvn hthec4"><div class="fbg_text3"><span class="fbpzol" title="Положительных отзывов о форекс брокере '.$fb->fname.'">'. $fb->fpotz .'</span><span class="fbpgray" title="Нейтральных отзывов о форекс брокере '.$fb->fname.'">'. $fb->fnotz .'</span><span class="fbpblue" title="Отрицательных отзывов о форекс брокере '.$fb->fname.'">'. $fb->footz .'</span></div></div>';
	    }
	    if($distable['enable']['fdc5']==1){
